...
from src.config.prompt import CONTEXTUALIZE_QUERY_PROMPT, SYSTEM_PROMPT
from src.engine.generate.formatter import OutputFormatter
from src.engine.generate.testcase import TestCaseGenerator
from src.engine.generate.usecase import UseCaseGenerator
from src.engine.guards.guard import GuardOrchestrator
from src.infra.metrics import metrics_collector
from src.infra.session import SessionManager
...
class Generator:
        ...
        try:
            with timer(timer_name):
                ...
                context_result = self.retriever.retrieve_with_context(
                    query=processing_query, top_k=top_k, search_mode=search_mode
                )
                retrieval_validation = self.guards.validate_retrieval(
                    context_result["chunks"], processing_query
                )
                ...
                if not retrieval_validation["is_sufficient"]:
                    return {
                        ...
                        "clarification": retrieval_validation.get(
                ...
                result = generation_func(
                    ...
                    context_result["avg_score"],
                )
                if not result.get("success", False):
                    return result

                output_validation = self.guards.validate_output(
                    self.formatter.to_json_string(result),
                    context_result["chunks"],
                    processing_query,
                )
                result["validation"] = {
                ...
                if session_id:
                    self.session_manager.add_turn(
                        session_id, query, summary_func(result), raw_json=result
                    )
    ...
    def _generate_combined_logic(
        self, query: str, chunks: list, avg_score: float
    ) -> Dict[str, Any]:
        context_text = "\n---\n".join([c["content"] for c in chunks])
        prompt = get_generation_prompt(
            query=query, mode="both", context_chunks=context_text, avg_score=avg_score
        )
        response = self.llm_client.generate(
        ...
        if not parsed:
            return self.formatter.create_error_response(
                query, "parse_error", "Failed to parse response"
            )
    ...
    def get_generator_stats(self) -> Dict[str, Any]:
        return {
            ...
            "metrics": {
                "use_cases_generated": metrics_collector.get_counter(
                    "use_cases_generated"
                ),
                "test_cases_generated": metrics_collector.get_counter(
                    "test_cases_generated"
                ),
                "combined_generated": metrics_collector.get_counter(
                    "combined_generated"
                ),
            },
        }
