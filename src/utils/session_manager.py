...
class SessionManager:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            try:
                cls._instance.mongo_client = pymongo.MongoClient(settings.MONGO_URI, serverSelectionTimeoutMS=2000)
    ...
    def _load_session(self, session_id: str) -> bool:
        """Attempts to load a session from MongoDB. Returns True if successful."""
        ...
        try:
            ...
            if doc and "history" in doc:
                self.sessions[session_id] = deque(doc["history"], maxlen=self.max_history)
                return True
        except Exception as e:
    ...
    def get_all_sessions(self) -> List[Dict]:
        ...
        if self.collection is None:
            return []
        try:
            docs = self.collection.find({}, {"_id": 1, "created_at": 1, "updated_at": 1, "history": 1})
            return list(docs)
        except Exception as e:
            print(f"Error fetching sessions: {e}")
            return []
    
    def add_turn(self, session_id: str, user_query: str, ai_response: str, raw_json: Optional[Dict] = None):
        if session_id not in self.sessions:
            if not self._load_session(session_id):
                self.sessions[session_id] = deque(maxlen=self.max_history)
        
        now = datetime.now(timezone.utc).isoformat()
        self.sessions[session_id].append({"role": "user", "content": user_query, "timestamp": now})
    ...
    def clear_session(self, session_id: str):
        ...
        if self.collection is not None:
            try:
                ...
                print(f"Error deleting session {session_id} from MongoDB: {e}")
